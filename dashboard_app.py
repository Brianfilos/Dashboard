...
if excel_file is not None:
    ...
    descripciones_gastos = [
        "COMISION PAGO A OTROS BANCOS", "COBRO IVA PAGOS AUTOMATICOS", "IVA COMIS TRASL SUC VIRTUAL",
        "COMISION TRASL SUC VIRTUAL", "COMISION PAGO A PROVEEDORES", "COMISION PAGO DE NOMINA",
        "CUOTA MANEJO TARJETA PREPAGO", "IVA POR COMISIONES CORRIENTE", "CUOTA MANEJO SUC VIRT EMPRESA",
    ...
    st.write(f"Suma total de Salidas utilizadas para gastos bancarios: {suma_salidas_gastos}")

    registro_gastos_bancarios = df_excel[(~df_excel['cruzado']) & 
    ...
    if not registro_gastos_bancarios.empty:
        ...
        st.write(f"Diferencia entre la suma del CSV y el registro del Excel: {diferencia_gastos}")
    ...
    registro_servicios_bancarios = df_excel[(~df_excel['cruzado']) & 
    ...
    # Actualizar DataFrames finales y mostrar los resultados
    df_cruzados = pd.DataFrame(registros_cruzados)
